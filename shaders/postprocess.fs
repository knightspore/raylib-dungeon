...
void main() {
    ...
    for (int x = -size; x <= size; x++) {
        for (int y = -size; y <= size; y++) {
            ...
            vec4 colorR = texture(tex, fragTexCoord + offset + vec2(chromaticAberration, 0.0) * dist * separation);
            ...
            if (valueR > threshold) colorR += colorR * (valueR - threshold);
            if (valueG > threshold) colorG += colorG * (valueG - threshold);
            if (valueB > threshold) colorB += colorB * (valueB - threshold);
